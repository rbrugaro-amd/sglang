...
def _notify(locks, increment: gl.constexpr, active, publish: gl.constexpr):
    ...
    if publish:
        gl.inline_asm_elementwise(
            ...
            [locks, increment, active.to(gl.int32)], dtype=gl.uint64,
...
def _scalar_acquire(counter, target, thread):
    ...
    gl.inline_asm_elementwise(
        ...
        dtype=(gl.uint32, gl.uint32), is_pure=False, pack=1,
    )
...
def _vector_acquire(counter, target, thread, materialize: gl.constexpr):
    ...
    compare: gl.constexpr = "v_cmp_ne_u32 vcc, 0, $4" if materialize else "v_cmp_eq_u32 vcc, 0, $4"
    gl.inline_asm_elementwise(
        compare + """
        s_and_saveexec_b64 $0, vcc
        s_cbranch_execz 2f
        ...
        2:
        s_or_saveexec_b64 $0, $0
        """, "=&s,=&v,v,v,v,~{vcc},~{scc},~{memory}", [counter, target, condition],
        dtype=(gl.uint64, gl.uint32), is_pure=False, pack=1,
    )
...
def _reserve_epoch(local, thread, step: gl.constexpr):
    ...
    _, ticket = gl.inline_asm_elementwise(
        ...
        [local, step, thread], dtype=(gl.uint64, gl.uint32),
        is_pure=False, pack=1,
    )
    return ticket
...
def _advance_single_row(local, next_progress, thread):
    ...
    gl.inline_asm_elementwise(
        ...
        s_or_saveexec_b64 $0, $0
        """, "=&s,v,v,v,~{vcc},~{scc},~{memory}",
        [local, next_progress, thread], dtype=gl.uint64, is_pure=False, pack=1,
    )
...
def _finish(local, target, last):
    gl.inline_asm_elementwise(
        ...
        s_or_saveexec_b64 $0, $0
        """, "=&s,=&v,v,v,v,~{vcc},~{scc},~{memory}",
        [local, target, last], dtype=(gl.uint64, gl.uint32),
        is_pure=False, pack=1,
    )
...
def _finish_late(local, target, thread, step: gl.constexpr):
    gl.inline_asm_elementwise(
        ...
        [local, step, target, thread], dtype=(gl.uint64, gl.uint32, gl.uint32),
        is_pure=False, pack=1,
    )
...
def _finish_uniform_late(local, target, thread, step: gl.constexpr):
    gl.inline_asm_elementwise(
        ...
        dtype=(gl.uint64, gl.uint32, gl.uint32), is_pure=False, pack=1,
    )
...
def _load_pointer(table):
    return gl.inline_asm_elementwise(
        ...
        "=s,s,~{memory}", [table.to(gl.uint64)], dtype=gl.uint64,
        is_pure=False, pack=1,
    )
...
def _load_inputs(table, owner):
    return gl.inline_asm_elementwise(
        ...
        [table.to(gl.uint64), owner * 8, ((owner+1)%4)*8,
         ((owner+2)%4)*8, ((owner+3)%4)*8],
...
@gluon.jit
def _fused(
    input_peer_ptrs, lock_peer_ptrs, residual_ptr, weight_ptr,
    normalized_ptr, residual_out_ptr, quantized_ptr, scales_ptr, reduced_ptr,
    M: gl.constexpr, ROUND: gl.constexpr, FP8_MAX: gl.constexpr, EPS: gl.constexpr,
):
    ...
    if TICKET:
        ...
        if M == 2:
            local_address, a0, a1, a2, a3 = _load_tables(lock_peer_ptrs, input_peer_ptrs)
        ...
        progress = gl.inline_asm_elementwise(
            ...
            "=v,v,~{memory}", [local], dtype=gl.uint32, is_pure=False, pack=1,
        )
    ...
    if M <= 2:
        reduced = gl.inline_asm_elementwise(
            "", "=v,0,~{memory}", [reduced], dtype=gl.bfloat16,
    ...
    if M != 4:
        _store_local(
            scales_ptr, rows[:, None] * 16 + groups[None, :], scales,
            M <= 2 or M == 16,
        )
    if M <= 8:
...
def tp4_allreduce_add_gemma_rmsnorm_group_fp8_quant_gluon(
    ...
    *,
    ...
    arch = torch.cuda.get_device_properties(local_input.device).gcnArchName.split(':', 1)[0]
    assert arch == 'gfx950'
    rounding = 'rtne'
    ...
    _fused[(m // 2 if m == 64 else m,)](
        input_peer_ptrs, lock_peer_ptrs, residual, gemma_weight,
        normalized, residual_out, quantized, scales, reduced,
        m, rounding, fp8_max, eps,
